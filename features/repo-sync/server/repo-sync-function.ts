//inngest workflow sambhal raha hai ye
...
export const syncRepoCodebaseFunction = inngest.createFunction(
    ...
    async ({ event, step }) => {
        ...
        // Step 1: Mark status as syncing in DB
        ...
        // Step 2: Fetch raw files via GitHub Octokit and chunk them
        ...
        // Step 3: If previously synced, delete the old namespace in Pinecone
        ...
        // Step 4: Batch upload vectors to Pinecone
        await step.run("save-vectors-to-pinecone", async () => {
            await saveRepoChunks(namespace, chunks);
        });

        // Step 5: Mark status as synced and update metadata
